package com.manna.audio.widget;
...
public class WaveView extends View {
     ...
     * 频谱数量
     */
    private static final int LUMP_COUNT = 99;
    //线条宽度
    private static final int LUMP_WIDTH = 2;
    //线条间隔
    private static final int LUMP_SPACE = 7;
    private static final int LUMP_MIN_HEIGHT = 10;
    ...
    private static byte[] readyData(byte[] fft) {
        ...
        int len = Math.min(fft.length, LUMP_COUNT);
        for (int i = 0; i < len; i++) {
            int absValue = Math.abs(fft[i]);
            newData[i] = (byte) absValue;
        }
        return newData;
    }
    ...
    private void drawLump(Canvas canvas, int i, boolean reversal) {
        ...
        canvas.drawRect(LUMP_SIZE * i, top, LUMP_SIZE * i + LUMP_WIDTH, LUMP_MAX_HEIGHT,
                lumpUpPaint);
    }
